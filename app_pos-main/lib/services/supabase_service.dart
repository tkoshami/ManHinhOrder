...
// ── Báo cáo Nhập - Xuất - Tồn ──
  ...
  // ─── DÁN ĐOẠN NÀY VÀO BÊN TRONG class SupabaseService (supabase_service.dart) ───
// (không thay cả file — chỉ thêm các hàm mới này vào trước dấu `}` cuối class)

// ─── QUẢN LÝ BÀN CHO MÃ QR ─────────────────────────────
  static Future<List<Map<String, dynamic>>> getQrTables() async {
    try {
      final rows = await _supabase.from('qr_tables').select().order('name');
      return List<Map<String, dynamic>>.from(rows);
    } catch (e) {
      print('getQrTables error: $e');
      return [];
    }
  }

  static Future<bool> createQrTable(String name) async {
    try {
      await _supabase.from('qr_tables').insert({'name': name});
      return true;
    } catch (e) {
      print('createQrTable error: $e');
      return false;
    }
  }

  static Future<bool> updateQrTable({required int id, required String name}) async {
    try {
      await _supabase.from('qr_tables').update({'name': name}).eq('id', id);
      return true;
    } catch (e) {
      print('updateQrTable error: $e');
      return false;
    }
  }

  static Future<bool> deleteQrTable(int id) async {
    try {
      await _supabase.from('qr_tables').delete().eq('id', id);
      return true;
    } catch (e) {
      print('deleteQrTable error: $e');
      return false;
    }
  }
}